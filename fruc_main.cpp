...
int main()
{
	...
	if((resized_test_out = fopen("resizedframe.yuv", "wb")) == NULL) {
		printf("Error : Output file could not be opened\n");
		return 1;
	}
	...
	{	
		...
		/*	DEBUG - write first resized frame to file		*/
		//for (i=0;i<(Config::image_height+2*Config::fs_win);i++){
		//	fwrite(&ResizedFrame2[i*(Config::image_width+2*Config::fs_win)],sizeof(unsigned char), (Config::image_width+2*Config::fs_win), resized_test_out);
		//}
		/*													*/
	...
	else
	{
		if(Config::enable_rep || up_conv_algo == 6)
		{
			...
			fprintf(results,"PSNR: N/A\t\tSAD Count: %d\t cycle: %d\n",SADcount,cyclecount);
	}
...
/* IMPORTANT!! READ BEAFORE USE !!																				*/
/* In this function the resulting MVs are pointing from the current block to the previous block					*/
/* In the original software MVs point from previous block to current block										*/
/* This modification reduces the complexity of the hardware design of the algorithm								*/
/* To match the results with HW simulation use the functions as given											*/
/* However to match the original software results change all of the lines containing:							*/
/* refData[R_INDEX((rowpos*Config::blocksize+candidates[i].y),colpos*Config::blocksize+candidates[i].x)]);		*/
/* to:																											*/
/* refData[R_INDEX((rowpos*Config::blocksize-candidates[i].y),colpos*Config::blocksize-candidates[i].x)]);		*/
/* and the lines containing:																					*/
/* refData[R_INDEX((rowpos*Config::blocksize+tempvector[i].y),colpos*Config::blocksize+tempvector[i].x)]);		*/
/* to:																											*/
/* refData[R_INDEX((rowpos*Config::blocksize-tempvector[i].y),colpos*Config::blocksize-tempvector[i].x)]);		*/
/****************************************************************************************************************/
vector calcNewBlock3DRS(unsigned char* curData, unsigned char* refData, int rowpos, int colpos,int s_count,int ext_s_count, vector* s_locs, vector* ext_s_locs){
	...
	if (Config::enable_early_t && calculateSAD(&curData[R_INDEX(rowpos*Config::blocksize,colpos*Config::blocksize)],&refData[R_INDEX(rowpos*Config::blocksize,colpos*Config::blocksize)]) == 0){
		...
		//calculate median		
		whichvector = L1_MedianFilter(tempvector,3);
		vmedian = tempvector[whichvector];
		UWpos = vmedian;
		...
		if(under_threshold)
			...
			// limit MVs to Search Window
			if (candidates[1].x < -36)
			{
				candidates[1].x = -36;
			} 
			if (candidates[1].y < -36)
			{
				candidates[1].y = -36;
			} 
			if (candidates[1].x > 36)
			...
			for(i=0;i<2;i++){
				...
				{
					//if((abs(candidates[i].x - vmedian.x) <4 ))
					//{
					//	if ((abs(candidates[i].y - vmedian.y) <4 ))
					//	{

					//	}
					//	/*else if ((abs(candidates[i].y - vmedian.y) < 22 ))
					//	{
					//		cyclecount = cyclecount + abs(candidates[i].y - vmedian.y);
					//	}*/
					//	else
					//	{
					//		cyclecount = cyclecount + 22;
					//	}
					//	
					//}
					///*else if ((abs(candidates[i].y - vmedian.y) <4 ))
					//{
					//	if ((abs(candidates[i].x - vmedian.x) < 22 ))
					//	{
					//		cyclecount = cyclecount + abs(candidates[i].x - vmedian.x);
					//	}
					//	else
					//		cyclecount = cyclecount + 22;
					//}*/
					//else
					//{
					//	/*if (abs(candidates[i].x - vmedian.x) + abs(candidates[i].y - vmedian.y) < 22)
					//	{
					//		cyclecount = cyclecount + abs(candidates[i].x - vmedian.x) + abs(candidates[i].y - vmedian.y);
					//	}
					//	else*/
					//		cyclecount = cyclecount + (16*16);
					//}
		...
		else
		{
			...
			if (whichvector == 0)
			{
				tempvector[1] = update(tempvector[1]);
				if (tempvector[1].x < -36)
				{
					tempvector[1].x = -36;
				} 
				if (tempvector[1].y < -36)
				{
					tempvector[1].y = -36;
				} 
				if (tempvector[1].x > 36)
			...
			{
				tempvector[0] = update(tempvector[0]);
				if (tempvector[0].x < -36)
				{
					tempvector[0].x = -36;
				} 
				if (tempvector[0].y < -36)
				{
					tempvector[0].y = -36;
				} 
				if (tempvector[0].x > 36)
			...
			else
			{
				...
				tempv = tempvector[whichvector];
				tempvector[whichvector] = tempvector[0];
				tempvector[0] = tempv;
			}
			tempvector[s_count].x = 0;
			...
			for(i=0;i<s_count;i++){  //use for(i=0;i<=s_count;i++) to include zero motion vector
				...
				{
					if((abs(tempvector[i].x - UWpos.x) <4 ))
					{
						if ((abs(tempvector[i].y - UWpos.y) <4 ))
						{

						}
						else if ((abs(tempvector[i].y - UWpos.y) < 22 ))
						{
							cyclecount = cyclecount + (abs(tempvector[i].y - UWpos.y))*22;
							UWpos.y = tempvector[i].y;
						}
						else
						{
							cyclecount = cyclecount + (22*22);
					...
					else if ((abs(tempvector[i].y - UWpos.y) <4 ))
					{
						if ((abs(tempvector[i].x - UWpos.x) < 22 ))
						{
							cyclecount = cyclecount + (abs(tempvector[i].x - UWpos.x))*22;
							UWpos.x = tempvector[i].x;
						}
						else
						{
							cyclecount = cyclecount + (22*22);
					...
					{
						/*if (abs(tempvector[i].x - vmedian.x) + abs(tempvector[i].y - vmedian.y) < 22)
						{
							cyclecount = cyclecount + abs(tempvector[i].x - vmedian.x) + abs(tempvector[i].y - vmedian.y);
						}
						else*/
							cyclecount = cyclecount + (22*22);
							UWpos = tempvector[i];
...
/*and writes the motion vectors into motion mectors array and also into a file		*/
/*----------------------------------------------------------------------------------*/
void calcFrame3DRS(unsigned char* curData, unsigned char* refData,FILE* outfile,FILE* MV_X,FILE* MV_Y,int s_count,int ext_s_count, vector* s_locs, vector* ext_s_locs,FILE* SAD_File){
	...
	for (i=0;(i<(BLOCK_COUNT_OF_COLUMN) && i < 67);i++){
		for (j=0;j<(BLOCK_COUNT_OF_ROW);j++){
			...
			fprintf(outfile,"(%3d,%3d)\n",mvectors[(i*(BLOCK_COUNT_OF_ROW))+j].x,mvectors[(i*(BLOCK_COUNT_OF_ROW))+j].y);
...
unsigned char* ResizeFrame (unsigned char* orgininalFrame)
{
	...
	for (i=0;i<ResizedHeight;i++){
		for (j=0;j<ResizedWidth;j++){
			...
			//fprintf(resizedFILE,"%02X ",ResizedFrame[i*ResizedWidth+j]);
		}
		//fprintf(resizedFILE,"\n");
...
int L1_MedianFilter(vector* VectorSet,int VectorCount)
{
	int temp_median;
	int min_median = 9999;
	int Which_Vector = 0;
